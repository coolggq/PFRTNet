...
class DataTransform(object):
    ...
    def __call__(self, kspace, mask, target,  fname, slice_num): # attrs,
        """
        Args:
        ...
        Returns:
            (tuple): tuple containing:
                ...
                slice_num (int): Serial number of the slice.
        """
        kspace = to_tensor(kspace) # (640,358,2)
        # image = ifft2_2c(kspace)
        # inverse Fourier transform to get zero filled solution
        image = ifft2c(kspace) # 640,368,2 torch.complex64
        ...
        image = complex_center_crop(image, crop_size) # 320,320,2 torch.complex64

        # getLR       #SR任务，仅减小分辨率
        imgfft = fft2c(image) # 320,320,2 torch.complex64
        imgfft = complex_center_crop(imgfft, (128,128)) # 160,160,2 torch.complex64
        LR_image = ifft2c(imgfft) # 160,160,2 torch.complex64

        # absolute value
        LR_image = complex_abs(LR_image)  # (160,160) tensor,torch.complex64;complex_abs()输入必须是复数，且最后一维是2

        # normalize input
        LR_image, mean, std = normalize_instance(LR_image, eps=1e-11)
        LR_image = LR_image.clamp(-6, 6)  # LR_image现在是torch.complex64的类型，.clamp不能处理
...
class ReconstructionTransform(object):
    ...
    def __call__(self, kspace, mask, target, attrs, fname, slice_num):
        ...
        image1 = image
        image2 = complex_abs(image1)  # (320,320)

        # getLR
        imgfft = fft2c(image)
        imgfft = complex_center_crop(imgfft, (160, 160))   # 两倍时，输入都是160×160；四倍时，输入80×80
        ...
        # normalize input
        LR_image, mean1, std1 = normalize_instance(LR_image, eps=1e-11)
        ...
        if self.mask_func:
            ...
            masked_kspace, mask = apply_mask(imgfft, self.mask_func, seed)
        else:
            masked_kspace = kspace

        LR_image_m = ifft2c(masked_kspace)

        # absolute value
        LR_image_m = complex_abs(LR_image_m)

        # normalize input
        LR_image_m, mean, std = normalize_instance(LR_image_m, eps=1e-11)
        LR_image_m = LR_image_m.clamp(-6, 6)
        ...
        return LR_image_m, target, mean, std, fname, slice_num, LR_image, image2
        # # apply mask
        # if self.mask_func:
        #     seed = None if not self.use_seed else tuple(map(ord, fname))
...
def build_transforms(args, mode = 'train', ):
    ...
    if args.WORK_TYPE == 'sr':
        ...
        if mode == 'train':
            return DataTransform(args.DATASET.CHALLENGE) # 超分辨率任务，就没有欠采样过程
        elif mode == 'val':
